...
* @file
*/
namespace Drupal\entrants\Form;

use Drupal\Core\Form\FormBase;
use Drupal\Core\Form\FormStateInterface;
use Drupal\Core\Url;

class EntrantsForm extends FormBase { 
	/**
   	* {@inheritdoc}
   	*/
 	public function getFormId() {
   		return 'entrants_form';
  	}
	...
	function download($From_Date, $To_Date) { 
		...
		if (trim($From_Date) == "" & trim($To_Date) == "") {
			  ...
			  $entrant_query = $entrant_con->select('Entrants', 't')
        		->fields('t', ['Id', 'Date_Of_Entry', 'First_Name', 'Surname', 'Email', 'Telephone', 'Code', 'Please_send_me_any_further_offers'])	
				->orderBy('t.Id','Desc');
		}				
		else {
			if (trim($From_Date) !== "" & trim($To_Date) == "") {			

...
class EntrantsRegistrantsForm extends FormBase { 
	/**
   	* {@inheritdoc}
   	*/
 	public function getFormId() {
   		return 'entrants_registrants_form';
  	}
	...
	function download($From_Date, $To_Date) { 
		...
		if (trim($From_Date) == "" & trim($To_Date) == "") {
			  ...
			  $entrant_query = $entrant_con->select('Registrants', 't')
        		->fields('t', ['Id', 'Date_Of_Entry', 'First_Name', 'Surname', 'Email', 'Pin'])	
				->orderBy('t.Id','Desc');
		}				
		else {
			if (trim($From_Date) !== "" & trim($To_Date) == "") {			
		...
		foreach ($result as $row) {			
			...
			$First_Name = trim(stripslashes($row->First_Name));											 								 			
		...
		header("Content-Disposition: attachment; filename=anchor_coupon_registrants.csv");
